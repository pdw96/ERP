...
> **설계도 메뉴 절이 요구하나 42판 구현에 없던 칸 넷** — 부분군 크기 ·
> 상시 목표 Cpk(1.33) · 전환 게이트 Ppk(1.67) · WE 규칙 켬/끔. 넷 다 관리도와
> 공정능력이 서는 단계(8단계)에서 읽히는 값이다. **1단계에 둘지는 미결**이며,
> 두지 않기로 하면 그 단계에서 마이그레이션이 하나 는다.
...
- `CHECK` 재작업은 자사 로트에만
- **양방향 CHECK**(지적 ①) — 재고구분과 창고의 조합을 양쪽에서 막는다
