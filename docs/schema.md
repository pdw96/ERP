...
(「오늘」이 나오면 파이썬). 14 부터는 거래 표라 **비어 있는 것이 정상**이다 —
로트를 채우는 것은 판정이고, 그 판정과 근거와 사실을 적는 셋이 2단계에서 섰다.
...
> 재고조정으로 숨는다. 견줄 실적이 생기는 5단계의 일이므로 **두지 않기로
> 했다.**
...
> 통과한다. 같은 표의 다른 줄을 보는 조건은 CHECK 로 적을 수 없다. **표를
> 세우던 동안에는** 시드가 유일한 쓰기 경로라 정합 테스트가 지켰고, 화면에서
> 코드를 만드는 길이 생기는 날 쓰기 시점 검증이 함께 서야 한다.
...
| `unit` | `mm` · `cP` · `ΔE` 처럼 **재는 단위.** 재고 단위(`UOM`)와 다른 축이라 공통코드를 가리키지 않는다 |

**재는 기준에는 단위가 있다.** `CHECK (상한과 하한이 둘 다 비었거나) OR unit IS NOT NULL` — 규격이 하나라도 있으면 그 기준은 재는 것이고, 재는데 단위가 없으면 그 숫자는 뜻을 갖지 못한다. **그리고 이것이 없으면 17번의 단위 잠금이 통째로 새어 나간다** — 측정 줄은 단위를 이 칸에서 가져오고, 복합 외래키는 한 칸이라도 `NULL` 이면 검사하지 않는다. 세는 항목(`이물` · `포장` · `성적서`)은 재지 않으므로 그대로 비어서 선다.

**그리고 단위에 뜻이 있다.** `CHECK unit IS NULL OR is_present(unit)` — 빈 문자열과 탭 · 전각 공백은 위의 `IS NOT NULL` 을 통과하면서 아무것도 말하지 않는다. **재는 기준만 묻지 않는 이유**는 17번에 있다 — 세는 기준에 빈 문자열이 앉으면 측정 줄의 빈 단위에 **짝이 생겨** 그쪽 쌍 외래키가 그것을 통과시킨다. 정한 사람이 없으면 `NULL` 이고, 빈 문자열은 그 자리의 값이 아니다.

**단위까지 가리킬 상대가 있다.** `UNIQUE NULLS NOT DISTINCT (process_code, item_code, material_group, unit)` — **행을 좁히지 않는다.** 위의 셋이 이미 유일하므로 넷째를 더해도 같은 줄이고, 목적은 17번의 측정 줄이 **단위까지 쌍으로 가리킬 상대**를 만드는 것이다. 그래서 가리키는 줄이 있는 동안 이 칸을 바꾸는 것 자체가 막힌다 — **거래 데이터가 자기 근거를 잠근다.** 이 유일키가 없으면 17번의 `applied_unit` 쌍 외래키가 설 자리를 잃는다.
...
| `id` | PK | |
| `item_id` | FK items | |
| `lot_number` | str(50) | **우리가 짓는다** — 아래 「로트 번호는 우리가 짓는다」. 공급사가 붙여 온 번호는 `inspections.supplier_lot_number` 에 남는다 |
| `lot_origin` | str(10) | 공급사 · 자사. **물건이 온 곳이지 번호의 출처가 아니다** |
...
| `inspection_id` | FK → `inspections` NULL | **자기를 만든 검사.** 특채 표식이 사는 곳 |
| `inspection_result` | str(10) NULL | 그 판정. 외래키의 자리이지 값을 나르는 칸이 아니다 |
...
품목 코드 · 입고일 · 그날의 일련번호. **창고 라벨에서 사람이 물건을 찾는
번호**이므로 번호만 보고 무엇이 언제 들어왔는지 읽힌다. 공급사가 붙여 온 번호는
`inspections.supplier_lot_number` 에 그대로 남는다 — 남이 지은 번호를 우리
유일키에 쓰면 두 공급사가 같은 번호를 쓸 때 둘째 입고가 거부된다.

**번호의 정렬에 기대지 않는다.** 일련이 두 자리를 넘으면 자릿수가 늘어 문자
정렬이 어긋나지만, FIFO 는 번호가 아니라 위의 날짜 칸이 든다.

### 불합격이 로트를 만들지 못하는 것을 무엇이 막는가

「그 검사가 불합격인가」는 **다른 표의 칸을 보는 조건**이라 CHECK 로 적을 수
없다. 판정을 이 줄에 들고 **쌍으로** 가리키면 그 줄만 보고 막을 수 있다 —
특채 플래그를 검사 기록에 건 것과 같은 방식이고, 칸은 외래키에 묶여 갈릴 수
없다. 검사 쪽 판정을 고치려 해도 이 줄이 가리키던 짝이 사라져 그 수정 자체가
막힌다(원칙 ⑦).

> **「사 온 로트에는 검사가 있다」는 걸지 않는다.** 걸어 봤더니 **기초재고가
> 막혔다** — 과거를 소급하지 않기로 했으므로 이월로 깔리는 자재 로트에는 적을
> 검사도 합격일도 없다. 그래서 검사를 **가리키는** 로트만 판정에 묶인다.
> 「검사 없이 선 자재 로트」와 「이월로 깔린 자재 로트」를 데이터베이스가 가르려면
> 이월을 표시할 자리가 있어야 하고, 그 자리는 **전기이월이 서는 조각**의 것이다.
...
도착일은 사람이 보내는 값인데 앉는 자리가 `lots.received_date` 하나였고, 원칙 ①
이 「불합격은 로트가 되지 않는다」이므로 **불합격 판정에서는 그 값이 통째로
버려졌다** — 201 로 성공 응답이 나가면서다. 하필 불합격이 **클레임과 반품의
근거**가 되는 판정이고, 그 근거의 첫 줄이 「언제 받은 물건인가」다.

`judged_at` 이 대신이 되지 못한다. 이 조각은 뒤늦게 적은 입고를 일부러 받으므로
(미래만 막는다) 둘은 며칠씩 갈리고, **그 차이가 곧 검사가 늦은 날수**다.

**같은 사실이 두 표에 살게 되므로 쌍으로 묶는다.** `lots.received_date` 는 이월
로트(검사를 모른다)와 FIFO 정렬 키 때문에 남아야 하므로 지울 수 없다. 대신 로트가
`(inspection_id, received_date)` 로 검사를 가리켜 **갈릴 수 없게** 한다 —
...
## 16. `inspection_measurements` — 측정값 줄
...
- `FK (process_code, item_code, material_group, applied_unit) → process_inspection_standards (…, unit)` — **단위도 쌍으로.** 기준 쪽의 두 `CHECK`(**재는 기준에는 단위가 있다** · **단위에 뜻이 있다**)가 함께 서야 뜻을 갖는다 — 단위가 `NULL` 이면 복합 외래키가 그 줄에서 **통째로 건너뛰어지고**, 빈 문자열이면 건너뛰지는 않되 **그 빈 값에 짝을 만들어 준다.** 박아 두는 것만으로는 이 줄과 기준이 갈릴 수 있고, 갈린 쪽이 옳은지 말해 줄 것이 없다. 쌍으로 가리키면 **가리키는 줄이 있는 동안 기준의 단위를 바꾸는 것 자체가** 막힌다 — 아무도 가리키지 않는 기준은 그대로 고칠 수 있다

### 규격을 박아 두는 이유 — 파생값 금지의 그 예외다

예외는 하나다 — **「그 판정의 근거가 되었고 밖으로 나간 값」.** 규격은 고객이
정하고 **바뀐다.** 바뀐 뒤에 옛 검사를 열면 「측정값 30 · 합격」인데 지금 규격으로
재계산하면 불합격이 나온다 — 기록이 자기모순을 일으킨다.

그래서 **합격 여부가 아니라 그때 쓴 규격을 박는다.** 판정은 측정값과 박아 둔
규격에서 언제나 같게 계산되므로, 파생값을 저장하지 않으면서도 판정이 재현된다.

**그 적용이 한동안 절반이었다.** 상·하한은 박았는데 그 숫자가 `µm` 인지 `mm` 인지는
기준 표에만 있었다 — 거기서 단위를 고치면 측정값도 박아 둔 한계도 **하나도 바뀌지
않는데** 읽히는 뜻이 천 배 달라진다(50µm 가 50mm 가 된다). 숫자가 그대로라 아무도
알아채지 못하고, 그 기록이 고객에게 가거나 클레임 근거가 된다. `applied_unit` 이
그 나머지 절반이다 (Codex 리뷰 NC-122).

### 분말에 점도를 재지 않는다 — 사슬이 세 표를 묶는다

품목이 자재군을 알고, **검사가 품목에서 그것을 받아 들고**, 측정 줄이 그 검사와
같은 자재군으로만 기준을 가리킨다. 세 표가 한 줄로 묶이므로 **품목의 무리가 아닌
기준은 가리킬 수 없다** — 자재군 축이 기준 표에서 연 것을 한 겹 아래에서 닫는
자리다. 묶지 않으면 그 축이 한 층 아래에서 무효가 된다.

그 사슬 때문에 `items` 와 `inspections` 에 유일키가 하나씩 는다. 둘 다 `id` 가
이미 기본키라 **행을 좁히지 않는다** — `uq_item_id_type` 과 같은 이유다.

### 공정을 못박는 CHECK 를 두지 않는다

기준 표에 「자재군이 있다 ⇔ 수입」이 **양방향으로** 걸려 있고, 이 줄은 자재군을
반드시 들고 기준을 가리킨다. 그래서 가리킬 수 있는 기준은 자재군이 있는 줄, 곧
수입 기준뿐이다. 같은 명제를 CHECK 로 한 번 더 적으면 관문 2 에서 한 자리가
남는다 — **테스트가 그 자리를 이름으로 지킨다.**

> **박아 둔 규격이 그때 그 기준의 값과 같은가는 제약으로 걸 수 없다.** 값까지
> 외래키에 넣으면 기준을 고치는 것 자체가 막히고, 그러면 규격이 바뀌어도 옛
> 판정이 남는다는 이 칸의 존재 이유가 사라진다. 제약이 걸리는 것은 **비어
> 있음**까지다(둘 다 비면 거부).
>
> **값이 같은 것은 쓰는 쪽이 지킨다.** 쓰는 쪽은 하나이고
> (`app/services/incoming.py`) 기준에서 **그대로 베낀다** — 고를 여지가 없으므로
> 갈릴 자리도 없다. 그것을 `tests/test_write_path.py` 의
> `test_the_measurements_pin_the_spec_they_were_judged_against` 가 이름으로
> 지킨다. **쓰는 쪽이 둘이 되는 날 이 자리가 다시 열린다.**

---

## 17. `stock_ledger_entries` — 수불 원장
...
|---|---|---|
| `id` | PK | |
| `lot_id` | FK → `lots` (`inspection_id` 와 쌍) | |
| `txn_type` | str(30) | **공통코드가 아니라 `txn_type_attributes` 를 가리킨다** |
| `quantity` | float | **늘 양수.** `>= 0` 과 `is_finite()` 를 함께 |
| `occurred_at` | datetime | |
| `inspection_id` | FK → `lots` (`lot_id` 와 쌍) | 입고 줄이 **그 로트를 만든** 검사를 가리킨다 |
...
- `CHECK txn_type = '구매입고'` — 이 조각이 내는 유형은 하나다. 나머지 열하나는 내는 쪽이 3~8단계에 있고, 받아 두면 **근거 문서가 없는 줄**이 선다
...
감소인데 수량이 양수인 줄」을 제약이 막지 못한다. 부호를 쓰면 `>= 0` 을 걸 수
없다는 것도 값이다 — `is_finite()` 단독은 `NaN` 은 막아도 **음수 입고**는 막지
않는다.

**「조정」이 양방향인 것은 이 조각에서 풀지 않는다.** 그 유형이 나는 자리는
재고조정이고 범위 밖이다 — 부르는 쪽이 없는 칸을 지금 지어내지 않는다.

### 유일 인덱스에 조건을 적은 이유

`UNIQUE (lot_id)` 로 두면 오늘은 같은 뜻이다. 다만 불출이 서는 날 한 로트에 여러
줄이 나야 하므로 그때 이 제약을 손봐야 하고, **손봐야 하는 제약은 손보지 않은
채로 남는다.** 유형을 조건에 적어 두면 그날에도 그대로 옳다.

### 입고 줄은 그 로트를 만든 검사를 가리킨다

`FK (lot_id, inspection_id) → lots (id, inspection_id)` — **쌍으로** 가리킨다.
둘을 따로 가리키면 둘 다 실재한다는 것까지만 증명되고, 남의 검사를 가리키는
입고 줄이 서면 「왜 이 물건이 들어왔는가」로 내려가는 길이 엉뚱한 판정에 닿는다.

이 쌍은 표가 설 때는 만들 수 없었다 — 그때 로트가 검사를 몰랐기 때문이다.
**쓰기 경로가 서면서 닫혔다.** 그러면서 `inspection_id` 를 **홀로** 가리키던
외래키는 떨어져 나갔다 — 위의 칸 표가 한동안 그것을 계속 적고 있었고(NC-69),
그것을 독립 외래키로 읽으면 「로트 없이 검사만 가리키는 원장 줄」이 설 수 있다고
오해한다.
...
| ~~**로트가 어느 공급사에서 왔는지 모른다**~~ — **정해졌다: 로트 번호를 우리가 짓는다.** 공급사 번호는 검사 쪽에 남고 `lots` 에 공급사 칸을 더하지 않는다 — 로트가 `inspection_id` 로 검사를 가리키고 검사가 공급사를 안다. 근거는 `schema-2단계.md` 의 「로트 번호는 우리가 짓는다」 | **닫힘** — 2단계 설계에서 결정. 위 `lot_number` · `lot_origin` 설명은 **CodeRabbit 리뷰가 낸 뒤에야 따라갔다**(NC-111) — 「함께 간다」고 적어 둔 것이 함께 가지 않았다 |
| 검사 항목이 몇인가 — 설계도는 18, 공정별 표는 17, **불합격을 적을 수 있는 것만 세면 16**이다 | 폭이나 치수를 따로 재기로 할 때 |
| **같은 검사를 두 번 보내면 재고가 두 벌 선다** — 쓰기 경로에 멱등 키가 없고 `(공급사 × 공급사 로트번호)` 에 유일 제약도 없다. **막지 않기로 정했다**: 한 공급사 로트가 여러 번 나뉘어 오는 **분할 납품**이 실제로 있고, 전역이든 공급사로 좁히든 유일키는 그것을 함께 막는다. 「정하지 않았다」가 아니라 **정하고 적어 둔다** (감사 ⑧ NC-67) | **상쇄 수불유형이 서는 단계(3~8).** 지금 막지 않기로 한 대가가 「잘못 선 입고를 되돌릴 길이 없다」인데, 그 길은 구매입고 하나만 받는 이 원장에 없다 — 상쇄가 서는 날 둘이 함께 답을 갖는다 |
| **로트 수량과 입고 줄 수량을 묶는 것이 애플리케이션 코드뿐이다** — 같은 수량이 검사 · 로트 · 원장 셋에 나뉘어 들어가는데 셋을 묶는 제약이 없다. 오늘 갈리지 않는 것은 쓰는 코드가 하나뿐이라서이고, **그것은 제약이 아니라 우연이다.** 제안된 모양(`lots` 에 `UNIQUE (id, quantity)` 를 두고 원장이 쌍으로 가리킨다)은 **쓰지 않는다** — 외래키는 유형을 가리지 못해 **출고 줄에도 걸리고**, 로트 100 에서 50 을 빼는 줄이 그 자리에서 거부된다 (감사 ⑧ NC-70) | **출고가 서는 단계.** 「그 로트의 줄들의 **합**이 로트 수량인가」는 줄 하나의 제약이 아니라 합의 규칙이라, 뺄 줄이 서 봐야 형태가 보인다 |
| **공급사–품목 구매 관계를 수입검사가 묻지 않는다** — `supplier_items` 에 그 쌍이 없어도 입고가 선다. **막지 않기로 정했다**: 수입검사는 **물건이 실제로 도착했다는 사실**을 적는 자리이고, 구매 마스터가 없다는 것은 **발주 쪽의 결함**이지 도착하지 않았다는 뜻이 아니다 — 긴급 구매와 대체 공급사는 실제로 일어난다. 여기서 막으면 표가 이미 일어난 일을 부정한다(「제약이 사실을 막으면 안 된다」 — 기초재고에서 한 번 겪었다) (Codex 리뷰 NC-108) | **발주가 서는 3단계.** 구매 단위 · 환산 계수 · 리드타임이 필요해지는 자리이고, 「발주 없는 입고를 어떻게 볼 것인가」와 같은 질문이다 |
| ~~**불합격 검사의 입고일이 어디에도 남지 않는다**~~ — **정해졌다: 칸을 더한다.** `inspections.received_date` 가 섰고(§15), 로트의 같은 칸과 **쌍으로 묶여** 갈리지 못한다. 옛 줄은 합격한 것만 로트에서 옮겨 오고 불합격은 비어서 선다 — 없는 값을 지어내지 않는다 (Codex 리뷰 NC-109) | **닫힘** — 저자가 답했고 `a7c14b3e9052` 가 냈다 |
...
| `tenant_id` | 단일 테넌트 |
| ~~수불 원장~~ | **섰다 — §17.** 로트가 생기는 것이 곧 재고가 생기는 것이라 함께 섰다 |
| ~~검사 기록 · 측정값 줄~~ | **섰다 — §15 · §16.** 판정과 그 근거가 앉는 표다 |
