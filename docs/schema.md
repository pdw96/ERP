...
**재는 기준에는 단위가 있다.** `CHECK (상한과 하한이 둘 다 비었거나) OR unit IS NOT NULL` — 규격이 하나라도 있으면 그 기준은 재는 것이고, 재는데 단위가 없으면 그 숫자는 뜻을 갖지 못한다. **그리고 이것이 없으면 `inspection_measurements` 의 단위 잠금이 통째로 새어 나간다** — 측정 줄은 단위를 이 칸에서 가져오고, 복합 외래키는 한 칸이라도 `NULL` 이면 검사하지 않는다. 세는 항목(`이물` · `포장` · `성적서`)은 재지 않으므로 그대로 비어서 선다.

**그리고 단위에 뜻이 있다.** `CHECK unit IS NULL OR is_present(unit)` — 빈 문자열과 탭 · 전각 공백은 위의 `IS NOT NULL` 을 통과하면서 아무것도 말하지 않는다. **재는 기준만 묻지 않는 이유**는 `inspection_measurements` 에 있다 — 세는 기준에 빈 문자열이 앉으면 측정 줄의 빈 단위에 **짝이 생겨** 그쪽 쌍 외래키가 그것을 통과시킨다. 정한 사람이 없으면 `NULL` 이고, 빈 문자열은 그 자리의 값이 아니다.
...
| ~~**검사 기준이 품목을 가리지 못한다**~~ — **정해졌다: 자재군을 둔다.** 품목 축을 더하면 원자재 15 × 검사항목만큼의 실측값을 누군가 정해야 하고, 그 값이 없으면 빈 기준정보가 된다. 자재군은 **품목 시드가 이미 적어 둔 구분**이다(분체 다섯 · 액상수지 여섯 · 시트필름 넷) — 시드에 근거가 있어 지어낼 값이 없다. 재고단위는 그 경계를 반만 말한다(`M2` 넷 = 시트필름 넷이고 `KG` 아홉은 두 무리로 갈린다). 초안은 `schema-2단계.md` 의 자재군 절(번호로 가리키지 않는다 — 그 문서 머리가 이유를 든다) | **닫힘** — 2단계 착공에서 결정 |
