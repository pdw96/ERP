...
- `FK (process_code, item_code, material_group, applied_unit) → process_inspection_standards (…, unit)` — **단위도 쌍으로.** 박아 두는 것만으로는 이 줄과 기준이 갈릴 수 있고, 갈린 쪽이 옳은지 말해 줄 것이 없다. 쌍으로 가리키면 **가리키는 줄이 있는 동안 기준의 단위를 바꾸는 것 자체가** 막힌다 — 아무도 가리키지 않는 기준은 그대로 고칠 수 있다
