...
> **미결 — 이 줄이 가리키는 검사가 그 로트를 만든 검사인가.** 데이터베이스가
> 아직 보증하지 못한다. 묶으려면 로트가 자기를 만든 검사를 가리켜야 하는데
> (`lots.inspection_id`), 그 칸은 **로트를 실제로 만드는 쪽**이 서는 조각에서
> 붙는다 — 지금 붙이면 아무도 채우지 않는 칸이 선다.
>
> 같은 조각이 「**로트 생성과 원장 줄이 한 트랜잭션**」과 「**불합격이 로트를
> 만들지 못한다**」도 닫는다. 둘 다 쓰는 쪽이 있어야 말할 수 있는 것이고,
> 오늘은 로트를 만드는 길 자체가 없다.
