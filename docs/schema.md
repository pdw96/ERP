...
| **불합격 검사의 입고일이 어디에도 남지 않는다** — 합격은 로트가 `received_date` 를 들지만 불합격은 로트가 서지 않는다. `judged_at` 은 대신이 되지 못한다(이 조각은 **뒤늦게 적은 입고를 일부러 받는다**). 고치려면 `inspections` 에 칸을 더해야 한다 (Codex 리뷰 NC-109) | **저자가 답한 뒤** — 데이터 모델은 앵커볼트라 먼저 묻는다 |
