...
| **로트가 어느 공급사에서 왔는지 모른다** — 두 공급사가 같은 품목에 같은 로트 번호를 쓰면 `UNIQUE (item_id, lot_number)` 가 두 번째 입고를 거부하고, 반품 흐름도 어느 공급사인지 가릴 수 없다 | **지금 열려 있다** — 2단계 조각 안이며 ID 체계(앵커볼트)를 건드리므로 구현 전에 묻는다 |
