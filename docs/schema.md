...
> **미결 — 박아 둔 규격이 그때 그 기준의 값과 같은가.** 이것은 **제약으로 걸 수
> 없다.** 값까지 외래키에 넣으면 기준을 고치는 것 자체가 막히고, 그러면 규격이
> 바뀌어도 옛 판정이 남는다는 이 칸의 존재 이유가 사라진다. 걸리는 것은
> **비어 있음**까지다(둘 다 비면 거부). 값이 같은지는 **쓰는 쪽의 몫**이며,
> 쓰기 엔드포인트가 서는 조각에서 닫는다.
