...
| **검사 기준이 품목을 가리지 못한다** — PK 가 `(공정 × 검사항목)` 이라 「수입」 기준 여덟이 **원자재 열다섯 전부에** 똑같이 걸린다. 자재는 한 덩어리가 아니다 — 재고단위가 그것을 말한다(KG 아홉 · L 둘 · M2 넷). 분말에 점도를, 라이너에 입도를 재라고 내미는 셈이다. 품목 축을 더할지 자재군을 둘지는 **읽는 쪽(IQC 화면)이 서는 2단계에서 정한다** | 관문 1 이 서는 2단계 |
