...
특채를 여는 사유로만」이 주석이 아니라 외래키로 걸린다. 셋째로 **측정값 줄**이
섰다 — 판정 시점의 규격을 박아 두어, 기준이 바뀌어도 옛 판정이 재현된다. 넷째로
**수불 원장**이 섰다 — 수량은 늘 양수이고 방향은 유형이 말한다.

다섯째로 **쓰기 경로**가 섰다 — 검사 한 건을 받아 판정하고, 합격이면 로트와
원장 줄을 **한 트랜잭션으로** 만든다. **2단계의 범위가 다 섰다.**
...
돌리고 끝나는 일회성 잡이며, 그것이 끝난 뒤 `api` 가 뜬다 — 엔드포인트는
