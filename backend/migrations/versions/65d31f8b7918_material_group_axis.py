...
이상에 걸치는 항목만 **그 줄을 베껴** 나머지 무리에 세운다.

**옮긴 줄과 베낀 줄은 다르다.** 옮기는 것은 줄이지 `UPDATE` 가 고르는 칸이 아니라,
옛 줄에 사람이 넣은 값이 들어 있었다면 **그대로 따라간다** — 고객이 준 규격도, σ 도
출처가 「실측」이든 「임의」든 같다. 베낀 줄은 **규격만 따라오고 σ 는 「미정」으로
선다** — 그 무리에서는 σ 가 정해진 적이 없기 때문이다(아래 `INSERT` 의 주석).
...
**이 리비전은 표를 다시 쓰고, 도는 동안 세 표의 쓰기가 멈춘다.** 통과하는 것과
...
# 원자재 열다섯이 어느 무리인가. **품목 시드가 이미 적어 둔 구분이다**
# (`seed_data/03_items.sql` — 분체 다섯 · 액상수지 여섯 · 시트필름 넷).
...
# 되돌리기 가드가 **줄 대 줄로 견주는** 칸. σ 와 σ출처는 뺀다 — 그 둘은 **늘
# 갈리기 때문**이다. 베끼는 `INSERT` 가 새 무리 줄에 `NULL` 과 「미정」을 박으므로,
# 남길 줄에 σ 가 하나라도 있으면 두 줄은 반드시 그 칸에서 갈린다. 그 갈림은 사람이
# 넣은 것이 아니라 **이 리비전이 적은 것**이라 여기서 물면 멈추지 않아야 할 되돌리기가
# 멈춘다. 사람이 넣은 σ 는 앞선 가드가 따로 본다.
#
# **도로 넣지 않는다.** 이 두 칸을 비교에 넣으면 σ 를 잰 데이터베이스는 그날부터
# 되돌릴 수 없다. `test_downgrade_does_not_stop_for_a_sigma_on_the_row_it_keeps`
# 가 그 문을 지킨다.
...
#
# **여섯을 본다.** 이 리비전이 세우지 않은 수입 기준(남이 더한 것) · 남길 한 줄이
# 아닌 자리의 σ(사람이 넣은 값 — 「임의」와 「실측」 둘 다다. 「미정」만 빈 값이다) ·
# **남길 줄과 다른 규격 · 경고비 · 경시변화 · 단위** · 이 리비전이 심지 않은 자재군
# 코드 · 뜻이 달라진 그룹 · **사람이 고친 품목의 자재군 배정.** 여섯 다 「일어난 일은
# 지우지 않는다」에 걸린다. 머리말도 여섯이라 적는다 — 한 파일이 같은 것을 두 수로
# 말하면 어느 쪽이 낡은 것인지 다음 사람이 알 수 없다.
#
...
def upgrade() -> None:
    ...
    # 정하는 것이라 같은 항목이면 무리가 달라도 그대로 쓸 수 있다. σ 는 **그
    # 무리에서 정해진 적이 없다** — 실측이든 임의든 같다(임의도 그 무리의 규격과
    # 목표 Cpk 에서 역산한 값이라 무리가 바뀌면 다시 내야 한다). 베껴 오면 정한
    # 적 없는 것을 정했다고 적는 것이 되므로 「미정」으로 세운다(σ 와 출처를 묶는
    # 양방향 CHECK 가 그 짝을 지킨다).
    op.execute(
