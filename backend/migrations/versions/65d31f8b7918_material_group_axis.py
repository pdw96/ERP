...
이상에 걸치는 항목만 **그 줄을 베껴** 나머지 무리에 세운다. **사람이 넣은 값이
들어 있었다면 그대로 따라간다** — 고객이 준 규격도, σ 도 출처가 「실측」이든
「임의」든 같다. 옮기는 것은 줄이지 `UPDATE` 가 고르는 칸이 아니기 때문이다.
...
- **`code_groups` 는 세지 않는다.** 이 리비전이 그 표에 하는 일은 `INSERT` 뿐이라
  `RowExclusiveLock` 이고, 그것은 다른 쓰기를 막지 않는다 — DDL 을 받는 표
