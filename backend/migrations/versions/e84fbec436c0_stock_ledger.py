...
**`downgrade()` 가 원장을 통째로 지운다. 그래서 멈추고 이름을 말한다.** 되돌릴 수
없다 — 들어온 사실과 그 근거가 함께 사라지고 복구할 방법은 백업뿐이다. 처음에는
「이 리비전이 세운 표라 사람이 먼저 넣어 둔 값이 있을 수 없다」고 적고 멈추지
않았는데, 표가 이 리비전의 것이라는 것과 **줄이 이 리비전의 것**이라는 것은 다르다.

**뒤에 선 리비전의 가드에 기대지 않는다.** `08d406fa7f3b` 위에서는 원장 줄이 있으면
로트가 반드시 검사를 가리키므로 그쪽 가드가 먼저 문다. 하지만 **이 리비전이 머리인
데이터베이스**에는 그 가드가 없다 — 가드는 자기 리비전에서 사라지는 것을 본다.
...
def downgrade() -> None:
    # **사라지는 것을 먼저 이름으로 말하고 멈춘다.** 원장 줄은 로트가 생기고
    # 움직인 **사실**이며, 원칙 ⑦ 이 「일어난 일은 지우지 않는다」이다.
    op.execute(
        """
        DO $$
        DECLARE moved text;
        BEGIN
          SELECT string_agg(DISTINCT l.lot_number, ', ' ORDER BY l.lot_number) INTO moved
          FROM stock_ledger_entries AS e JOIN lots AS l ON l.id = e.lot_id;
          IF moved IS NOT NULL THEN
            RAISE EXCEPTION
              '되돌리면 이 로트들이 움직인 사실이 원장에서 사라진다: %. 잔량을 다시 세울 근거가 남지 않는다',
              moved;
          END IF;
        END $$;
        """
