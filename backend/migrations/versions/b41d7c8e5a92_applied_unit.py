...
**셋 다 조이기 전에 묻는다 — 위의 자리마다** — 제약이 먼저 걸리면 배포하는 사람이 무엇을 고쳐야
하는지 모른 채로 멈춘다(CodeRabbit 리뷰 NC-126).

**올릴 때 잠근다.** CHECK 가 `process_inspection_standards` 의 기존 행 전체를
검사하고, `SET NOT NULL` 이 `inspection_measurements` 를 전수 검사하며, 유일키가
인덱스를 만들고, 외래키는 **양쪽을 잡으므로** 이 리비전의 외래키가 가리키는 표는
전부 함께 잠기고 그동안 쓰기가 멈춘다(목록을 세지 않는다. 세면 갈린다).
`migrations/env.py` 가 전체를 트랜잭션 하나로 감싸므로 커밋까지 유지된다.

**칸을 더하는 것도 무해하지 않다 — 축이 다르다.** 기본값이 아예 없어 표를 다시
쓰지는 않지만, 그것은 **다시 쓰는가**의 답이지 **잠그는가**의 답이 아니다.
`ADD COLUMN` 은 ACCESS EXCLUSIVE 를 잡고 그것이 이 리비전의 **첫 구문**이므로, 그
잠금이 바로 뒤의 데이터 단계를 지나 커밋까지 유지된다 — 멈추는 길이를 정하는 것은
조이는 구문이 아니라 **행 수에 비례하는 그 구간**이다. `65d31f8b7918` 이 같은
자리를 먼저 적었다(감사 ⑭ NC-151).

**내릴 때도 멈출 수 있다 — 아래의 자리에서, 이름을 말하고.** 되돌리면 이 칸이 사라지지만 **그 사실을 다시 만들 수
