...
def upgrade() -> None:
    ...
    # 앞 스키마에서 `inspection_id` 가 찬 로트는 `lots.item_id` 가 `NOT NULL` 이라
    # **`fk_lot_inspection_item` 이 건너뛰어지지 않고**(복합 외래키는 한 칸이라도
    # `NULL` 이면 통째로 빠져나간다 — 바로 위가 그 부류다), 그 외래키 때문에
    # 검사와 품목이 같아야 하고, 그 검사는 `ck_inspection_item_is_raw_material` 로
    # 원자재이며, `fk_inspection_item` 과 `fk_lot_item` 이 `(item_id, item_type)` 을
    # `items` 에 각각 묶으므로 **로트의 품목종류도 원자재**라
    # `ck_lot_origin_matches_type` 이 공급사로 묶고,
