...
**`downgrade()` 는 표 열넷을 지운다. 되돌릴 수 없다.** 기준정보 전부와 로트가 함께
사라지고 복구할 방법은 백업뿐이다 — 백업 없이 돌리지 않는다.

한 표 옆의 `65d31f8b7918` 은 자재군 코드 **세 줄**을 지우기 전에 이름을 말하고
멈추는데, 그 앞의 이 리비전은 표 열넷을 아무 말 없이 지웠다. 부적합 하나를 고치고
같은 모양을 옆에 남기지 않는다.

**올릴 때도 잠근다** — `CREATE TABLE` 열넷과 인덱스가 새 표를 만든다. 비어 있는
데이터베이스에서만 도는 자리라 오늘은 잴 것이 없지만, 그 사실이 「잠그지 않는다」는
뜻은 아니다.
