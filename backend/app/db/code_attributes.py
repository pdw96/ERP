...
class NonconformityAttribute(Base):
    ...
    **목록을 두 벌 두지 않는다.** 열아홉 중 **열여덟**이 검사 항목을 가리키고
    (계량 열넷 · 계수 넷), 손으로 유지하는 것은 `IQ-EXP` 하나뿐이다 — 재는 값이
    아니라 입고일과 설정기간의 비교 결과라 가리킬 항목이 없다. 계량 코드가 항목을
    가리키면 둘이 갈릴 수 없다 — 항목에 「접착력」을 넣고 코드 표에 그것이 없으면
    불합격을 적을 수가 없고, 반대로 하면 쓰이지 않는 코드가 남는다.
    """
